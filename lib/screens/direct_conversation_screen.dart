...
class DirectConversationScreen extends StatefulWidget {
  ...
  @override
  State<DirectConversationScreen> createState() => _DirectConversationScreenState();
}
...
class _DirectConversationScreenState extends State<DirectConversationScreen> {
  ...
  void initState() {
    ...
    _messageBloc!.add(LoadDirectMessagesEvent(conversationId: widget.conversationId));
  ...
  void _onScroll() {
    if (_scrollController.position.pixels >= _scrollController.position.maxScrollExtent - 200) {
      ...
      if (state is MessageLoadedState && state.hasMore && !state.isSending) {
        context.read<MessageBloc>().add(LoadDirectMessagesEvent(
  ...
  void _sendMessage() {
    ...
    context.read<MessageBloc>().add(SendDirectMessageEvent(
      conversationId: widget.conversationId,
      body: text,
    ));
                  ...
                  BlocBuilder<MessageBloc, MessageState>(
                    builder: (context, state) {
                      if (state is MessageLoadedState) {
                        return Text(
                          state.isConnected ? 'Active' : 'Connecting...',
                          style: TextStyle(
                            fontSize: 11,
